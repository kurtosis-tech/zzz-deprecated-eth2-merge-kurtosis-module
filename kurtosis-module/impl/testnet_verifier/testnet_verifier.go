...
func LaunchAsynchronousTestnetVerifier(params *module_io.ExecuteParams, enclaveCtx *enclaves.EnclaveContext, elClientCtxs []*el.ELClientContext, clClientCtxs []*cl.CLClientContext) error {
	containerConfig := getAsynchronousVerificationContainerConfig(params, elClientCtxs, clClientCtxs)
...
func RunSynchronousTestnetVerification(params *module_io.ExecuteParams, enclaveCtx *enclaves.EnclaveContext, elClientCtxs []*el.ELClientContext, clClientCtxs []*cl.CLClientContext) (int32, string, error) {
	...
	cmd := getCmd(params, elClientCtxs, clClientCtxs, true)

	return svcCtx.ExecCommand(cmd)

}

func getCmd(params *module_io.ExecuteParams, elClientCtxs []*el.ELClientContext, clClientCtxs []*cl.CLClientContext, addBinaryName bool) []string {
	cmd := make([]string, 0)
	if addBinaryName {
		cmd = append(cmd, "./merge_testnet_verifier")
	}
	cmd = append(cmd, "--ttd 0")
	...
	cmd = append(cmd, "--ttd-epoch-limit 0")
...
func getAsynchronousVerificationContainerConfig(
	...
	cmd := getCmd(params, elClientCtxs, clClientCtxs, false)
